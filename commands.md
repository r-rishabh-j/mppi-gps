...
- `--reset-optim-per-iter` — wipe Adam m/v moments at end of each round. Useful when the buffer shifts distribution enough that stale momentum becomes a liability.
...
- `--reset-optim-per-iter` — rebuild the policy's Adam optimizer at end of each GPS iter, wiping m/v moments. Each GPS iter is a new supervised task (shifting C-step data) and stale momentum can blow through the `--prev-iter-kl-coef` trust region.
